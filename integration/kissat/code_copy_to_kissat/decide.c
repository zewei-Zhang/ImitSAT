...
#include "imitsat_client.h"   /* imitsat_next_decision */

#include <inttypes.h>
#include <string.h>

/* -------------------------------------------------------------------------- */
/* ImitSAT helpers (self-contained)                                           */
/* -------------------------------------------------------------------------- */

/* Convert internal literal to DIMACS ±(v+1). kissat uses Minisat-style       */
/* encoding: lit = 2*v + sign_bit, where sign_bit = 0 for positive, 1 for ~. */
static inline int
dimacs_of_lit (unsigned lit)
{
  unsigned v = (lit >> 1);
  return (lit & 1) ? -((int) v + 1) : ((int) v + 1);
}

/* Build dyn string from the current trail/frames:
 *
 *  Tokens:
 *    - "D" <lit> for decisions
 *    - <lit> for propagations on that level
 *  plus a final trailing " D".
 *
 *  Example after two levels:
 *    level 1: decision -3, propagated 7, 8
 *    level 2: decision  5
 *
 *  dyn = " D -3 7 8 D 5 D"
 */
static void
kissat_build_dyn_text_from_trail (kissat *solver, char *buf, size_t cap)
{
  if (!buf || cap == 0)
    return;

  /* No decisions yet -> model expects just " D". */
  if (solver->level == 0) {
    if (cap >= 3) {
      buf[0] = ' ';
      buf[1] = 'D';
  ...
  unsigned cur_level = solver->level;
  unsigned max_level = (cur_level > 0 ? cur_level - 1 : 0);
  ...
  const frame    *frames     = BEGIN_STACK (solver->frames);
  const unsigned *trail      = BEGIN_STACK (solver->trail);
  ...
  for (unsigned level = 1; level <= max_level; ++level) {
    unsigned start = frames[level].trail;
    unsigned end;

    if (level < max_level)
      end = frames[level + 1].trail;
    else
      end = (unsigned) trail_size;

    if (start >= end || start >= trail_size)
      continue;

    unsigned lit = trail[start];
    int d = dimacs_of_lit (lit);

    /* " D <d>" */
    int written = snprintf (buf + n, (cap > n ? cap - n : 0), " D %d", d);
    if (written > 0)
      n += (size_t) written;

    for (unsigned i = start + 1; i < end && i < trail_size; ++i) {
      lit = trail[i];
      int a = dimacs_of_lit (lit);
      written = snprintf (buf + n, (cap > n ? cap - n : 0), " %d", a);
      if (written > 0)
        n += (size_t) written;
    }
  }
  ...
  /* Final trailing " D". */
  (void) snprintf (buf + n, (cap > n ? cap - n : 0), " D");
}

/* -------------------------------------------------------------------------- */
/* Original Kissat decision code                                              */
/* (unchanged except for the small ImitSAT hook in kissat_decide()).          */
/* -------------------------------------------------------------------------- */
...
void
kissat_start_random_sequence (kissat *solver)
{
...
static unsigned
next_random_decision (kissat *solver)
{
  if (!VARS)
    return INVALID_IDX;
...
int
kissat_decide_phase (kissat *solver, unsigned idx)
{
...
kissat_decide (kissat *solver)
{
  ...
  /* Increase decision level (as in stock Kissat).*/
  solver->level++;
  assert (solver->level != INVALID_LEVEL);

  /* ---------------- ImitSAT HOOK: guided decision first ------------------ */
  if (solver->imopts.imitsat_enabled &&
      solver->imc.fd >= 0 &&
      (solver->imc.guide_limit <= 0 ||
       solver->imc.guided_used < solver->imc.guide_limit)) {

    char dyn[8192];
    kissat_build_dyn_text_from_trail (solver, dyn, sizeof dyn);

#ifdef LOGGING
    LOG ("ImitSAT dyn=\"%s\"", dyn);
#endif

    int ext_lit = 0; 
    if (imitsat_next_decision (&solver->imc, dyn, &ext_lit) == 0 &&
        ext_lit != 0) {

      int v = (ext_lit > 0) ? ext_lit : -ext_lit;
      if (v >= 1 && (unsigned) v <= VARS) {
        unsigned idx = (unsigned) (v - 1);
        unsigned lit = LIT (idx);
        if (ext_lit < 0)
          lit = NOT (lit);
        ...
        if (!VALUE (lit)) {
          ...
          LOG ("decide literal %s (imitsat)", LOGLIT (lit));
          kissat_assign_decision (solver, lit);

          solver->imc.guided_used++;
          STOP (decide);
          return;
        }
      }
  ...
  /* Default Kissat decision heuristic. */
  const unsigned idx = kissat_next_decision_variable (solver);
  const value phase = kissat_decide_phase (solver, idx);
  unsigned lit = LIT (idx);
  if (phase < 0)
    lit = NOT (lit);
