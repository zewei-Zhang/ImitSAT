...
#define OPTIONS \
  ...
  /* ---- ImitSAT CLI options ---- */ \
  OPTION (imitsat, 0, 0, 1, "enable ImitSAT server guidance") \
  OPTION (imitsat_limit, 3, 0, INT_MAX, "max number of guided decisions") \
  OPTION (imitsat_port, 8765, 1, 65535, "ImitSAT server TCP port") \
  OPTION (imitsat_timeout, 200, 1, 60000, "ImitSAT socket timeout (ms)") \
  /* ---------------------------- */ \
...
bool kissat_options_parse_arg (const char *arg, char *name, int *val_ptr);
